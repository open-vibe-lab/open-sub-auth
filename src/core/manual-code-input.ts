...
import { OAuthCallbackError, StateMismatchError } from "../errors.js";
 ...
 */
export function parseCodeAndState(
	input: string,
...
export function promptForCode(expectedState: string): Promise<AuthorizationResult> {
	return new Promise((resolve, reject) => {
		...
		rl.question(
			"Paste the authorization code (or code#state) from your browser: ",
			(answer) => {
				rl.close();
				try {
