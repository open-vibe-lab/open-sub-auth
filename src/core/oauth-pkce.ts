...
import { promptForCode } from "./manual-code-input.js";
...
export function buildAuthorizationUrl(
	...
	if (!config.authorizationEndpoint) {
		throw new OAuthCallbackError(
			`Provider "${config.name}" has no authorization endpoint`,
		);
	}
...
export async function executePKCEFlow(options: PKCEFlowOptions): Promise<TokenSet> {
	...
	return exchangeCode(config, authResult.code, codeVerifier, redirectUri);
}
