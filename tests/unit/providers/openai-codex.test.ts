...
describe("OpenAICodexProvider", () => {
  ...
  describe("config", () => {
    ...
    it("has correct authorization endpoint", () => {
      expect(provider.config.authorizationEndpoint).toBe("https://auth.openai.com/oauth/authorize");
    });

    it("has correct token endpoint", () => {
      expect(provider.config.tokenEndpoint).toBe("https://auth.openai.com/oauth/token");
    });
